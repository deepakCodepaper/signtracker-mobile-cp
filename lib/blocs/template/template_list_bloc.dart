...
class TemplateListBloc extends Bloc<TemplateListEvent, TemplateListState> {
  ...
  void getTemplateListByParameters(String duration, String lanes,
      String closure) {
    add(GetTemplatesByParameterEvent(
        duration, lanes, closure));
  }
  ...
  Stream<TemplateListState> mapEventToState(TemplateListEvent event) async* {
    if (event is GetTemplatesByParameterEvent) {
      ...
      final templates = await projectRepository.getTemplateDrawingNumber(
          event.drawingNumber);
      if (templates == null) {
        yield LoadingTemplatesFailedState();
      } else {
